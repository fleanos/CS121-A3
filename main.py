...
def compute(files, threadNum):
  ...
  for i in files:
    for token, freq in tokenize(getStrings(i)):
      if token in tokenDict: tokenDict[token].add((i, freq))
      else: tokenDict[token] = {(i, freq)}
  pickle.dump(tokenDict, open("Index\Index" + threadNum + ".txt", "wb"), pickle.HIGHEST_PROTOCOL)
  print(f"Process: {threadNum} - DONE")
